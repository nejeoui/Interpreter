...
public class RestTest extends InterpreterApplicationTests {
	...
	public void oneSessionTest() throws Exception {
	   String uri = "/exec";
	   //First Query x=30
	   InterpreterQuery query=new InterpreterQuery("x=30");
	   ...
	   //Second Query print x
	   	query=new InterpreterQuery("print x");
	...
	public void cleanInterpreterTest() throws Exception {
	   ...
	   //First Query x=45\nprint x
	   InterpreterQuery query=new InterpreterQuery("x=45\nprint x");
	   String inputJson = super.mapToJson(query);
	   ...
	   mvc.perform(MockMvcRequestBuilders.post(uri).session(session)
			      .contentType(MediaType.APPLICATION_JSON_VALUE)).andReturn();
	  
	    
	   
	    //clean the interpreter locals and execute print x
	    query=new InterpreterQuery("print x");
	    inputJson = super.mapToJson(query);
	    mvcResult = mvc.perform(MockMvcRequestBuilders.post(uriClean).session(session)
			      ).andReturn();
